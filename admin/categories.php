...
//$efqtree = new EfqTree($xoopsDB->prefix("efqdiralpha1_cat"),"cid","pid");
$dirtree = new EfqTree($xoopsDB->prefix("efqdiralpha1_dir"),"dirid",0);
...
function catConfig($get_dirid="0")
{
	global $xoopsDB, $xoopsModule, $xoopsUser, $mytree, $efqtree, $dirtree;
	if ($get_dirid == "0") {
		...
		if ($get_dirid == 0) {
			...
			xoops_cp_header();
			adminmenu(2,_MD_MANAGE_CATS);
			echo "<h4>"._MD_SELECTDIRECTORY."</h4>";
			echo "<table width='100%' border='0' cellspacing='1' class='outer'>";	
			echo "<table width='100%' border='0' cellspacing='1'><tr><td>";
			$form = new XoopsThemeForm(_MD_SELECTDIRFORM, 'submitform', 'categories.php', "get");
			$directories_array = $directoryHandler->getAllDirectoryIds(); 
			//Add parent category select box.
			ob_start();
			$pcat_selbox = $dirtree->makeMySelBox("name", "name", "", 0, "");
			$selbox = ob_get_contents();
			ob_end_clean();
			$dir_tray = new XoopsFormElementTray(_MD_FORWHICHDIRECTORY_MANAGE_CATEGORIES, "<br />");
			$dir_tray->addElement(new XoopsFormLabel("", $selbox));
...
{
	global $xoopsDB, $efqtree, $mytree, $mytree2, $xoopsUser, $get_catid, $eh, $myts, $xoopsModuleConfig;
	xoops_cp_header();
	$diridfromcatid = getDirId($get_catid);
	adminmenu(2,_MD_EDITCAT);
    ...
    if ( $numrows > 0 ) {
		while(list($cid, $dirid, $title, $active, $pid, $img, $allowlist, $showpopular, $imgheight, $imgwidth, $descr) = $xoopsDB->fetchRow($result)) {
			...
			$form->addElement(new XoopsFormFile(_MD_SELECT_PIC, 'img', $xoopsModuleConfig['catimagemaxsize']));
	...
	$fieldtypes = $fieldtype_handler->getByDir($diridfromcatid);
	foreach ($fieldtypes as $fieldtype) {
		$fieldtypes_arr[$fieldtype['typeid']] = $fieldtype['title'];
	}
	$selbox = new XoopsFormSelect("", "typeid", 0, 1);
	$selbox->addOptionArray($fieldtypes_arr);
	...
	$fieldtype_tray->addElement($selbox);
	$fieldtype_tray->addElement(new XoopsFormLabel("", " <a href=\"fieldtypes.php\">"._MD_EDIT.""));
...
function updateCat()
{
	...
	if ( $_POST['xoops_upload_file'][0] != "" ) {
		...
		$uploader = new XoopsMediaUploader(XOOPS_ROOT_PATH."/modules/$moddir/init_uploads", array('image/gif', 'image/jpeg', 'image/pjpeg', 'image/x-png', 'image/png', 'image/jpg'), $xoopsModuleConfig['catimagemaxsize'], $xoopsModuleConfig['catimagemaxwidth'], $xoopsModuleConfig['catimagemaxheight']);
...
function getCatOverview($dirid=0)
{
	global $xoopsDB, $myts, $eh, $mytree, $moddir;
	$mainresult = $xoopsDB->query("SELECT cid, title, active, pid FROM ".$xoopsDB->prefix("efqdiralpha1_cat")." WHERE dirid='".$dirid."' AND pid='0'");
