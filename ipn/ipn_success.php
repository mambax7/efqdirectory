...
if (isset($paypal['business'])) {
    ...
    if (true === checkDuplicateTrx($values['txn_id'], $values['payment_status'])) {
        $newid = $xoopsDB->genId($xoopsDB->prefix($helper->getDirname() . '_subscr_payments') . '_id_seq');
        $sql   = 'INSERT INTO ' . $xoopsDB->prefix($helper->getDirname() . '_subscr_payments') . "
...
function checkDuplicateTrx($txn_id = '0', $payment_status = '0')
{
    ...
    $userid      = 0;
    $sql         = 'SELECT txn_id, payment_status FROM ' . $xoopsDB->prefix($helper->getDirname() . '_subscr_payments') . " WHERE txn_id='$txn_id' AND payment_status='$payment_status'";
