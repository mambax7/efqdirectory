 ...
 * @version   1.1.0
 */
class DataTypeHandler extends \XoopsObjectHandler
{
     ...
     *
     * Sets the object created by class DataType with data from query
     *
    ...
    public function setDataType($gpc_dtypeid = 0)
    {
        ...
        if ($numrows > 0) {
            while (false !== (list($dtypeid, $title, $section, $fieldtypeid, $uid, $defaultyn, $activeyn, $options, $custom, $icon) = $this->db->fetchRow($result))) {
                if (!$this->objDataType) {
                    $this->objDataType = new Efqdirectory\DataType();
                }
