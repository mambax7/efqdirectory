 ...
 * @version   1.1.0
 */
class SubscriptionOfferHandler extends \XoopsObjectHandler
{
    //    public $db; //Database reference
    public $objOffer; // reference to object

    /**
     * SubscriptionOfferHandler::ItemTypeHandler()
     *
     * @param bool $offer
     */
     ...
     * @version   1.0.0
     *
     * @param   SubscriptionOffer $obj object
     *
    ...
    /**
     * SubscriptionOfferHandler::setOffer()
     *
     * Sets the object created by class SubscriptionOffer with data from query
     *
    ...
    public function setOffer($gpc_offerid = 0)
    {
        ...
        if ($numrows > 0) {
            while (false !== (list($offerid, $offertitle, $typeid, $duration, $count, $price, $activeyn, $currency, $descr, $dirid, $typename, $level) = $this->db->fetchRow($result))) {
                if (!$this->objOffer) {
                    $this->objOffer = new Efqdirectory\SubscriptionOffer();
                }
